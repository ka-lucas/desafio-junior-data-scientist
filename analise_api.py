    ...
    if holidays:
        for holiday in holidays:
            ...
            if date_holiday.weekday() < 5:
                ...
                print(f"O feriado {holiday["name"]} ocorreu no dia {days_week[date_holiday.weekday()]}")
    quantity = len(holidays_week)   
    return quantity
...
def weather_pred():
    ...
    if response.status_code == 200:
        ...
        return(most_common_weather_copy[["month", "weather_description"]].to_string(index=False))
    else:
...
def weather_on_holidays(year):
    ...
    if response.status_code == 200:
        ...
        if holidays:
            ...
            return("Não foi possível obter os feriados.")
    else:
        return(f"Erro ao buscar o clima: {response.status_code}, {response.text}")

#7. Houve algum feriado "não aproveitável" em 2024? Se sim, qual(is)?
def no_enjoy_holidays(year):
    no_enjoy_holidays = []
    df_holidays_weather = weather_on_holidays(year)
    holidays = get_holidays(year)
    ...
    return df_no_enjoy_holidays
   
        

#8. Qual foi o feriado "mais aproveitável" de 2024?
def best_holiday(year):
    best_holiday = [] 
    ...
    return (df_best_holiday)

year = 2024
print("------------------------------------------------------------------------------\n")
print("1. Quantos feriados há no Brasil em todo o ano de 2024?")
holidays = get_holidays(year)
print(f"A quantidade de feriados no Brasil em {year} é de {len(holidays)}\n")
print("------------------------------------------------------------------------------\n")
print("2. Qual mês de 2024 tem o maior número de feriados?")
month,max_holidays = holiday_quantity(year)
print(f"\nO mês com mais feriados é {month} com {max_holidays} feriados.\n")
print("------------------------------------------------------------------------------\n")
print("3. Quantos feriados em 2024 caem em dias de semana (segunda a sexta-feira)?")
quantity = holiday_week(year)
print(f"\nA quantidade total de feriados em dia de semana é de : {quantity}\n")
print("------------------------------------------------------------------------------\n")
print("4. Qual foi a temperatura média em cada mês?")
avg_temp = avarage_temp()
print(f"\nA temperatura média é de: \n{avg_temp}\n")
print("------------------------------------------------------------------------------\n")
print("5. Qual foi o tempo predominante em cada mês nesse período?")
response = weather_pred()
print(f"\nTempo predominante: \n{response}\n")
print("------------------------------------------------------------------------------\n")
print("6. Qual foi o tempo e a temperatura média em cada feriado de 01/01/2024 a 01/08/2024?")
holidays_weather = weather_on_holidays(year)
print(f"\nTempo e temperatura média : \n{holidays_weather}\n")
print("------------------------------------------------------------------------------\n")
print("7. Houve algum feriado não aproveitável em 2024? Se sim, qual(is)")
no_enjoy_holidays_ = no_enjoy_holidays(year)
print(f"\nFeriados não aproveitados: \n{no_enjoy_holidays_}\n")
print("------------------------------------------------------------------------------\n")
print("8. Qual foi o feriado mais aproveitável de 2024?")
enjoy_holidays = best_holiday(year)
print(f"\nMelhor feriado: \n{enjoy_holidays}\n")
